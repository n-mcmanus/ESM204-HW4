...
ggplot(data = scc_df, aes(x = discount_rates, y = scc_calc)) +
  ...
  geom_point(aes(x = 2.1, y = 71.35), color = "slateblue4", size = 2.5) +
  scale_x_continuous(breaks = seq(0, 10, by = 1))
```
...
### Question 5: Choosing between two policies
...
# convert to billions because number so large
exp_pv_policy_a_billions <- exp_pv_policy_a/1000000000

```

Policy A: Business as Usual

The expected present value of damages up to 2100 under policy A is \$`r exp_pv_policy_a_billions` billion.
...
# convert to billions because number so large
exp_pv_policy_b_billions <- exp_pv_policy_b/1000000000
```

Policy B: Strong Action on climate change

The expected present value of damages up to 2100 under policy B is \$`r exp_pv_policy_b_billions` billion.
...
ggplot(data = costs_df_long, aes(x = costs, y = total_cost)) +
  ...
  labs(x = "Initial Cost of Policy B ($)",
       ...
       title = "Total Costs of Policies A and B") +
  theme(title = element_text(face = "bold"))
